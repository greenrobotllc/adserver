              ...
              <div class="row">
                <div class="col-md-12">
                  <form class="form-horizontal" id="edit_add" method="POST">
                    ...
                    <div class="form-group">
                      <label for="name" class="col-sm-2 control-label" required>Ad Unit ID</label>
                      <div class="col-sm-10">
                        <input type="text" name="ad_unit_id" value="{{$data->ad_unit_id}}" class="form-control" id="ad_unit_id" placeholder="Name" minlength="3" maxlenght="50" required>
                      </div>
                    </div>
...
<input type="hidden" name="id" value="{{$data->id}}">
	  ...
	  <div class="form-group">
      <label for="adsense_zone" class="col-sm-2 control-label">MoPub Unit</label>
		  	...
		  	<option value="{{$z->id}}" 
				...
				if($z->id == $data->my_ad) {
					echo("selected");
						...
						?>
						>
						{{$z->name}} - {{$z->platform}} - {{$z->app}}</option>
          	@endforeach>
		  </select>
